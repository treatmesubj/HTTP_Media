...
def get_news_info(post_elems, subtext_elems):
    ...
    for index, elem in enumerate(post_elems):
        ...
        try:
            ...
            points = None
        try:
            comments_link = subtext_elems[index].select_one("a[href*='item']").get('href', None)
        except AttributeError:
            comments_link = None

        if points:
            news_info.append({'title': title, 'story_link': story_link, 'points': points, 'comments_link': f"https://news.ycombinator.com/{comments_link}"})
