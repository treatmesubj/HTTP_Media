...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in list(tf_options.keys()):
            ...
            print(
                f"{sys.argv[1]} is invalid option. Choose from {list(tf_options.keys())}"
            )
    else:
        tf_choice = "24hrs"
    ...
    posts_html = HTML(
        html=requests.get(
            f"https://old.reddit.com/r/whatstheword/top/{tf_options[tf_choice]}",
    ...
    post_link = (
        f"https://old.reddit.com{[link for link in posts[post_choice].links][0]}"
    )

    comments_html = HTML(
        html=requests.get(post_link, headers={"user-agent": "Mozilla/5.0"}).text
    )
    ...
    # comments
    threads = comments_html.find(
        "div.commentarea > div.sitetable > div[data-type='comment']"
    )
